...
if(isset($_POST["submit"])){
  ...
  $duplicate = mysqli_query($con,"SELECT * FROM `student`.`student_data` WHERE mail = '$email' OR reg = '$redg';") ;
  if(mysqli_num_rows($duplicate) > 0){
    $dis=true;
  }
  else{
    if($password == $confirmpassword){
      $query = "INSERT INTO `student`.`student_data` (`name`, `reg`, `mail`, `dob`, `pno`, `gen`, `dept`, `hob`, `pass`) 
